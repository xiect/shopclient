...
public abstract class BaseLogicalEntity<T> implements IntentPortable
{
     ...
     * 全て件数を削除。
     */
    public void deleteAll(DBHelper helper) {
        FWUtil.d("DB接続をオープンします。");
        SQLiteDatabase db = helper.getWritableDatabase();
        try{
            int count = db.delete( this.tableName(), null, null );
            FWUtil.d("DBからの削除に成功しました。件数：" + count);
        } finally {
            db.close();
        }
    }
     ...
     * 自身を削除。
     */
    public void deleteByCondition(DBHelper helper,String deleteCondition,String[] deleteArgs)
    {
        FWUtil.d("DB接続をオープンします。");
        SQLiteDatabase db = helper.getWritableDatabase();
        try{
            db.delete( this.tableName(), deleteCondition, deleteArgs );
            FWUtil.d("DBからの削除に成功しました。");
        } finally {
            db.close();
        }
